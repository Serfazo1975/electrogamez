...
const NOTIF: Record<string, (r: Repair) => string> = {
  diagnosing:    r => `Hola ${r.client}! 🔍 Tu equipo (${r.device}) en ElectroGamez está en DIAGNÓSTICO. Seguí el estado acá: ${SEGUIMIENTO} — Código: ${r.code}`,
  waiting_parts: r => `Hola ${r.client}! ⏳ Tu equipo (${r.device}) en ElectroGamez está ESPERANDO REPUESTOS. Seguí el estado acá: ${SEGUIMIENTO} — Código: ${r.code}`,
  in_progress:   r => `Hola ${r.client}! 🔧 Tu equipo (${r.device}) en ElectroGamez ya está EN REPARACIÓN. Seguí el estado acá: ${SEGUIMIENTO} — Código: ${r.code}`,
  ready:         r => `Hola ${r.client}! 👾 Tu equipo (${r.device}) en ElectroGamez ya está LISTO PARA RETIRAR. Seguí el estado acá: ${SEGUIMIENTO} — Código: ${r.code}`,
  completed:     r => `Hola ${r.client}! ✅ La reparación de tu equipo (${r.device}) en ElectroGamez quedó COMPLETADA. ¡Gracias por elegirnos! — Código: ${r.code}`,
  cancelled:     r => `Hola ${r.client}! Tu reparación (${r.device}) en ElectroGamez fue CANCELADA. Si tenés dudas escribinos por acá. — Código: ${r.code}`,
}
